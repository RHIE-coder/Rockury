...
export function registerMockingHandlers() {
  ...
  ipcMain.handle(CHANNELS.MOCK_APPLY, async (_event, args: { mockResult: IMockResult; connectionId: string }) => {
    try {
      const sql = mockingService.exportMock(args.mockResult, 'sql');
      if (!sql.trim()) {
        return { success: true, data: { appliedRows: 0 } };
      }
      const result = await queryService.executeQuery(args.connectionId, sql);
      return { success: true, data: { appliedRows: result.affectedRows ?? result.rowCount } };
    } catch (error) {
      return { success: false, data: null, error: (error as Error).message };
    }
  });
