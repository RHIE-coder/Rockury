-- =============================================================================
-- MySQL Test Database Init Script
-- Target: Docker MySQL 8+ (port 13306, user=test, password=test, db=testdb)
-- Purpose: Comprehensive coverage of all MySQL design elements
-- =============================================================================
...
--   cannot have FK constraints in MySQL. Referential integrity is handled
...
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    -- Address fields (no composite type in MySQL)
...
) ENGINE=InnoDB COMMENT='Product images with auto-increment sort_order';
...
-- NOTE: MySQL partitioned tables cannot have FK constraints.
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    -- Shipping address fields (no composite type in MySQL)
...
) ENGINE=InnoDB COMMENT='Orders partitioned by created_at (RANGE). FK to users omitted due to MySQL partition limitation.'
...
) ENGINE=InnoDB COMMENT='Inventory change audit trail, partitioned by changed_at. FK omitted due to MySQL partition limitation.'
...
-- --------------------------------------------------------------------------
-- 1.23 audit_logs (partitioned by performed_at)
-- --------------------------------------------------------------------------
...
) ENGINE=InnoDB COMMENT='Generic audit trail, partitioned by performed_at'
...
-- MySQL supports LIST partitioning but only on INT columns or expressions.
...
-- MySQL HASH partitioning requires INT expression. We use a hash of session_id.
-- --------------------------------------------------------------------------
...
-- Hash: equality-only lookup (MySQL uses BTREE adapter; USING HASH on InnoDB is memory hint)
-- InnoDB does not support true HASH indexes, but we create a BTREE index for the same purpose
CREATE INDEX idx_api_keys_key_value ON api_keys (key_value);

-- JSON indexes: MySQL 8 supports functional indexes on JSON expressions
...
-- Unique index on settings key (case-sensitive in MySQL by default; collation handles case)
...
-- Note: MySQL does not support materialized views.
...
CREATE FUNCTION fn_generate_slug(p_text TEXT)
RETURNS TEXT
DETERMINISTIC
NO SQL
BEGIN
...
CREATE PROCEDURE proc_cleanup_expired_notifications(IN p_older_than_days INT)
BEGIN
...
CREATE PROCEDURE proc_recalc_inventory(
...
CREATE TRIGGER trg_inventory_quantity_change
...
CREATE TRIGGER trg_users_before_delete_set_default_notifications
...
-- Create users (IF NOT EXISTS supported in MySQL 8)
...
-- Note: MySQL does not have pgcrypto. Using SHA2 for password hashing in test.
...
-- --------------------------------------------------------------------------
-- 8.11 Product Images (sort_order auto-incremented)
-- --------------------------------------------------------------------------
