...
import {
  ...
  Plus, Minus, Pencil, ChevronRight, GitCompareArrows, Workflow,
} from 'lucide-react';
import { Button } from '@/shared/components/ui/button';
import { Select } from '@/shared/components/ui/select';
import { useConnections } from '@/features/db-connection';
import { useDiagrams, useDiagramStore, useDiagramVersions } from '@/features/virtual-diagram';
import { MigrationWizard, MigrationPackList } from '@/features/migration-pack';
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-2">
        {/* View mode toggle: Compare / Migrate */}
        <div className="flex rounded-md border border-border">
          <button
            className={`flex items-center gap-1 px-2.5 py-1 text-xs font-medium transition-colors ${
              viewMode === 'compare' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground'
            }`}
          ...
          <button
            className={`flex items-center gap-1 border-l border-border px-2.5 py-1 text-xs font-medium transition-colors ${
              viewMode === 'migrate' ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground'
            }`}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {viewMode === 'compare' ? (
          ...
          /* ── Migrate Mode ── */
          selectedDiagramId && selectedConnectionId && diagramVersions && diagramVersions.length > 0 ? (
            <div className="space-y-6">
              ...
              />
              <MigrationPackList diagramId={selectedDiagramId} />
            </div>
          ) : (
            <div className="flex h-full items-center justify-center">
              <p className="text-sm text-muted-foreground">
                {!selectedDiagramId
                  ? 'Select a diagram to start migration.'
                  : !selectedConnectionId
                    ? 'Select a connection to start migration.'
                    : 'No diagram versions found. Save a version first to create migration packs.'}
              </p>
            </div>
          )
        )}
